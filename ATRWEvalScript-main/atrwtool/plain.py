...
def plot_cmc(cmc_values, query_count, label='CMC curve', marker=None, save_path=None, top_k=20):
    # Convert raw CMC counts to percentages
    # cmc_percentages = [value / query_count for value in cmc_values]
    cmc_percentages = [value / query_count for value in cmc_values[:top_k]] 
    
    # Plot the curve
    ranks = range(1, len(cmc_percentages) + 1)
    plt.plot(ranks, cmc_percentages, label=label, marker=marker)
    
    # Set plot info
    plt.xlabel('Rank')
    plt.ylabel('Recognition Rate')
    plt.title('CMC Curve (Query Expansion)')
    plt.grid(True)
...
def eval_impl(anno,res,partial=False):
    ...
    if partial:
        plot_cmc(cmc_sing, len(query_sing), label='Single-Cam CMC (Public Split)', marker='o', save_path='./graphs/single_cam_cmc1.png')
        plot_cmc(cmc_multi, len(query_multi), label='Cross-Cam CMC (Public Split)', marker='x', save_path='./graphs/cross_cam_cmc2.png')
    else:
        plot_cmc(cmc_sing, len(query_sing), label='Single-Cam CMC (Private Split)', marker='s', save_path='./graphs/single_cam_cmc3.png')
        plot_cmc(cmc_multi, len(query_multi), label='Cross-Cam CMC (Private Split)', marker='d', save_path='./graphs/cross_cam_cmc4.png')
